...
figure(2)
%subplot(3, 1, 2);
plot(tau, phi_xx, 'b-'); xlabel('Desplazamiento \tau (s)'); ylabel('\phi_{xx}(\tau)'); 
title('Función de Autocorrelación del Ruido Blanco'); grid on; xlim([0 0.5]);
...
xlabel('Frecuencia (Hz)'); ylabel('S_{xx}(f)'); 
title('Autoespectro de Potencia del Ruido Blanco'); grid on; xlim([-fs/2 fs/2]);