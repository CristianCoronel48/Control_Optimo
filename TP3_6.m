...
figure (3);
plot(xv);title('Parte fraccionaria de[(PI+Xi-1)^5]');
...
%xlabel('Tiempo \tau');
%subplot(2,1,2);
plot(fixx_nor(N:2*N-1));title('Autocorrelación de x');
xlabel('Tiempo \tau');
...
%xlabel('Pulsación en rad por seg'); ylabel('S_{xx}(w)');
%subplot(2,1,2);
semilogx(20*log10(abs(SxM_nor(1:N/2))),'.k');
title('Módulo de F(w) en dB'); grid on;
xlabel('Pulsación en rad por seg'); ylabel('S_{xx}(w)');